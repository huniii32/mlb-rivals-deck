...
export function SharePanel({
  decks, tables, activeId, links, setLinks, onSelectDeck, onImportDeck, onPreviewDeck,
}: {
  decks: Deck[];
  tables: SkillTables;
  activeId: string;
  links: RankLinks;
  setLinks: (next: RankLinks) => void;
    ...
    try {
      ...
      if (existing) {
        ...
        const { data: ok, error } = await supabase.rpc("update_ranking", {
          p_id: id, p_token: link.token, ...args,
        });
        if (error) throw error;
        if (ok) {
      ...
      if (typeof newId === "string") {
        setLinks({ ...base, [newId]: { token, deckId: active.id, sig: deckSig(args) } });
      }
      loadPub();
      alert("전체 랭킹에 등록됐습니다.");
    } catch (e) {
